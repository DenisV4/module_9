...
# -- Возможно вам понадобиться развернуть строку, чтобы это сделать
#    вам достаточно уметь проходить по строке циклом и использовать конкатенацию.
# -- Чтобы сложить символы строки в обратном порядке - подумайте над переменой мест слагаемых.
#    Будут ли результаты одинаковы если мы все символы будем добавлять в конец строки (строка = строка + символ) и если
#    мы будем их добавлять в начало строки (строка = символ + строка)?

filtered_input = input("Введите строку: ").replace(" ", "").lower()
reversed_input = ""

for char in reversed(filtered_input):
    reversed_input += char

print("Да, это палиндром!" if reversed_input == filtered_input else "Нет, это не палиндром!")
